...
struct ConversationView: View {
    ...
    var body: some View {
        ...
        .alert("File problem", isPresented: Binding(
            get: { fileError != nil },
        ...
        .sheet(isPresented: $showingStarter) {
            StarterPromptSheet(conversation: $conversation) { text in
                copy(text)
            }
        }
    ...
    private func submitAITurn() {
        ...
        guard !clean.isEmpty || !pendingAttachments.isEmpty else {
            return
        }

        let responseText = clean.isEmpty
            ? "[No pasted text. Files were attached for this turn.]"
            : clean
        ...
        saveEntry(
            role: .guidance,
            title: "NovaKit Guidance + Response Prompt",
            text: guidance.explanation + "\n\n--- NEXT PROMPT ---\n" + guidance.responsePrompt,
            attachments: pendingAttachments
        )
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack {
                ...
                Button {
                    ...
                    Label("Submit Turn", systemImage: "arrow.up.circle.fill")
                }
                .buttonStyle(.borderedProminent)
...
struct ChatTurnView: View {
    ...
    private var backgroundStyle: AnyShapeStyle {
        ...
        case .guidance:
            return AnyShapeStyle(Color.blue.opacity(0.18))
        case .starterPrompt, .followUpPrompt:
            return AnyShapeStyle(Color.purple.opacity(0.16))
        }
    }
}
...
struct StarterPromptSheet: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section {
                    Button("Generate Starter Prompt") {
                        ...
                        generated = PromptEngine.makeStarter(
                            goal: draftGoal,
                            type: conversation.promptType,
                            config: conversation.templateConfig
                        )
                        conversation.entries.append(
                            ConversationEntry(
                                role: .starterPrompt,
            ...
            .onAppear {
                draftGoal = conversation.userGoal
            }
        }
    }
}
        ...
        NavigationStack {
            Form {
                ...
                Section("Prompt Template") {
                    ...
                    Stepper(
                        "History turns: \(conversation.templateConfig.maxHistoryEntries)",
                        value: $conversation.templateConfig.maxHistoryEntries,
                        in: 1...20
                    )
                }
            }
            ...
            .onAppear {
                tagText = conversation.tags.joined(separator: ", ")
            }
        }
    }
